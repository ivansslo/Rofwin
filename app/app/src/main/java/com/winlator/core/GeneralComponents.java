...
public abstract class GeneralComponents {
    ...
    private static final String INSTALLABLE_COMPONENTS_URL = com.winlator.BuildConfig.CONTENT_BASE_URL+"installable_components/%s";
    ...
    private static void downloadComponentFile(final Type type, final String filename, final Spinner spinner, final String defaultItem) {
        ...
        String obbEntry = "installable_components/"+type.lowerName()+"/"+filename;
        if (ObbRuntimeManager.extractEntry(activity, obbEntry, destination)) {
    ...
    private static void showDownloadableListDialog(Type type, final Spinner spinner, final String defaultItem) {
        ...
        String obbContent = ObbRuntimeManager.readTextEntry(activity, "installable_components/"+type.lowerName()+"/index.txt");
        if (obbContent != null) {
            activity.runOnUiThread(() -> {
                preloaderDialog.close();
                showDownloadableListDialogContent(activity, type, spinner, defaultItem, obbContent);
            });
            return;
        }
        ...
        HttpUtils.download(String.format(INSTALLABLE_COMPONENTS_URL, type.lowerName()+"/index.txt"), (content) -> activity.runOnUiThread(() -> {
            preloaderDialog.close();
            if (content != null) {
                showDownloadableListDialogContent(activity, type, spinner, defaultItem, content);
            }
            else AppUtils.showToast(activity, R.string.a_network_error_occurred);
        }));
    }

    private static void showDownloadableListDialogContent(Activity activity, Type type, Spinner spinner, String defaultItem, String content) {
