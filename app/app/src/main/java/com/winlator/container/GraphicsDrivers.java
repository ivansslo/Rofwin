...
public abstract class GraphicsDrivers {
    ...
    public static boolean isVulkanDriver(String identifier) {
        return identifier != null && (identifier.equals(NONE) || identifier.equals(TURNIP) || identifier.equals(VORTEK));
    }
    ...
    public static String[] getItems(String apiName) {
        if (apiName.equalsIgnoreCase("VULKAN")) {
            return new String[]{getName(NONE), getName(TURNIP), getName(VORTEK)};
        }
    ...
    public static String getDefaultDriver(Context context) {
        if (GPUHelper.isLowEndMali(context)) {
            return GraphicsDrivers.NONE+","+GraphicsDrivers.VIRGL;
        }
